...
const createPromise = (position, delay1)=>{
    const promise = new Promise((res, rej)=>{
        ...
        setTimeout(()=>{
            if (shouldResolve) res(promiseObj);
            else rej(promiseObj);
        }, delay1);
    });
    return promise;
...
const createPromises = (event)=>{
    ...
    for(let i = 0; i < promisesData.amount; i++)createPromise(i + 1, promisesData.delay + i * promisesData.step).then(({ promisePosition , promiseDelay  })=>{
        (0, _notiflixNotifyAio.Notify).success(`✅ Fulfilled promise ${promisePosition} in ${promiseDelay}ms`, {
            timeout: 3000
        });
    }).catch(({ promisePosition , promiseDelay  })=>{
        (0, _notiflixNotifyAio.Notify).failure(`❌ Rejected promise ${promisePosition} in ${promiseDelay}ms`, {
            timeout: 3000
        });
    });
};
